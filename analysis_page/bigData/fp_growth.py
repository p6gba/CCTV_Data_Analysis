...
def find_frequent_itemsets(transactions, minimum_support, include_support=False):
    ...
    def clean_transaction(transaction):
        ...
        transaction = sorted((v for v in transaction if v in items),
        reverse = True, key=lambda v: items[v])
        return transaction
...
class Mongo:
    ...
    def Find_fp_Mongo(self):
        ...
        collection = database.flows2
        docs = collection.find({},{'Flow':1, '_id' : 0}).limit(20000)
        FlowDataList = []
        for line in docs:
...
if __name__ == '__main__':
    connection = Mongo()
    transactions = connection.Find_fp_Mongo()


    result = []
    for itemset, support in find_frequent_itemsets(transactions, 800, True):
